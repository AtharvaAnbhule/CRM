...
const PipelineView: React.FC<PipelineViewProps> = ({
  ...
  const handleAddLane = () => {
    setOpen(
      <CustomModal
        title="Create a Mini-Workflow"
        subTitle="Mini-Workflow allow you to group tickets in case of any information to store."
        scrollShadow={false}
      >
    ...
    <DragDropContext onDragEnd={onDragEnd}>
      <div className="bg-white/60 dark:bg-background/60 rounded-md p-4 use-automation-zoom-in">
        <div className="flex items-center gap-15 justify-between ">
          ...
          <Button
            className=" items-center flex left-10 bg-violet-700 hover:bg-violet-500 "
            onClick={handleAddLane}
          >
            <Plus className="w-4 h-4" /> Create Mini-Workflow
          </Button> 
        ...
        {!allLanes.length && (
          <div className="flex items-center justify-center w-full flex-col gap-2 text-muted-foreground pb-10">
            ...
            <p className="text-xs font-medium">
              You don&apos;t have any Mini-Workflow. Go create one!
            </p>
          </div>
        )}
      </div>
