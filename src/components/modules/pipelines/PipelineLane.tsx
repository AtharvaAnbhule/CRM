...
const PipelineLane: React.FC<PipelaneLaneProps> = ({
  ...
  const handleEditLane = () => {
    setOpen(
      <CustomModal title="Edit Mini-Workflow Details" scrollShadow = { false} >
  ...
  const handleDeleteLane = async () => {
    try {
      ...
      await saveActivityLogsNotification({
        agencyId: undefined,
        description: `Deleted a Mini-Workflow | ${response?.name}`,
        subAccountId,
      });

      toast.success("Deleted", {
        description: "Deleted Mini-Workflow",
      });

      router.refresh();
    } catch (error) {
      toast.error("Oppse!", {
        description: "Could not delete the Mini-Workflow",
      });
    }
  };
