  ...
  const handleClickCreatePipeline = () => {
    setModalOpen(
      <CustomModal
        ...
        subTitle="Workflow allows you to group tickets into lanes and track your busines processes all in one place."
        scrollShadow={false}
      >
        <CreatePipelineForm
            ...
            <Command className="w-full">
              ...
              <CommandGroup className="w-full">
                {pipelines.map((pipeline) => (
                  <Link
                    key={pipeline.id}
                    href={`/account/${subAccountId}/workflow/${pipeline.id}`}
