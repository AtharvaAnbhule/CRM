...
import { getPipelineDetails, getUserPipelines } from "@/queries/workflow";
import {
...
const PipelineIdPage: React.FC<PipelineIdPageProps> = async ({ params }) => {
  ...
  if (!pipelineId) redirect(`/account/${subaccountId}/workflow`);

  const pipelineDetails = await getPipelineDetails(pipelineId);
  if (!pipelineDetails) {
    redirect(`/account/${subaccountId}/workflow`);
  }
